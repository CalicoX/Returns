...
- **Q4 左右布局（2026-09-22 Park，覆盖前一日对调）**：`.rt-quad-cell.q4 .rt-bento-meter` absolute 到右侧垂直居中（`right: clamp(28px,3vw,48px)`、`width: min(46%,260px)`），对位 Q3 时钟；数字独占左下流内贴底，与 Q3 `7×24h` 基线自然对齐（DOM 实测 bottom 差 0）。**选择器必须带 `.q4`**（三个 class 才能压过共享规则 `.rt-quad-cell .rt-bento-meter` 的 `position: relative`，两个 class 压不过）；≤640 单列档回退 `position: static` 流内竖排，同样带 `.q4`。JSX 顺序不变（meter 在前、`.rt-bento-main` margin-top:auto 在后）。
