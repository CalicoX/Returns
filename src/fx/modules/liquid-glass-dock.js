  ...
  function createLiquidGlassDock(els) {
    ...
    function updateDockTheme() {
      ...
      try {
        ...
        // 1) 几何：中心点仍在深色 section 内
        if (darkByCenterPoint(cx, cy)) {
          dark = true;
        } else {
          // 2) 多点采样（不改 visibility，避免 race / 粘状态）
          var samples = [
            [cx, cy],
            [cx, Math.max(1, cy - 8)],
            [Math.max(1, cx - r.width * 0.28), cy],
          ...
          for (s = 0; s < samples.length; s++) {
            ...
            if (themeFromNode(node) === "dark") darkVotes++;
            else lightVotes++;
          }
          // 须明确多数深色才反色；平局 / 浅色多数 → 浅色（清掉粘住）
          dark = darkVotes >= 3 || (darkVotes > lightVotes && darkVotes >= 2);
        }

        // 始终显式开关，禁止只 add 不 remove
    ...
    // Lenis 平滑滚动时 native scroll 可能不触发，挂统一回调
    ...
    if (window.__lenis && typeof window.__lenis.on === "function") {
      try {
        window.__lenis.on("scroll", scheduleTheme);
      } catch {}
    }
    // 首屏主题（无 setInterval 轮询，靠 scroll/Mutation 足够）
    requestAnimationFrame(function () {
      updateDockTheme();
      requestAnimationFrame(updateDockTheme);
    });
