...
(function () {
  ...
  var move = [];
  var c;
  for (c = h2.firstChild; c; c = c.nextSibling) move.push(c);
  for (var mi = 0; mi < move.length; mi++) solid.appendChild(move[mi]);
  ...
  function absorbWords() {
    ...
    Array.prototype.forEach.call(words, function (w) {
      if (w.parentNode !== solid) solid.appendChild(w);
    });
    // 词之间的空格 text node
    var nodes = [];
    for (var n = h2.firstChild; n; n = n.nextSibling) {
      if (n !== solid && n !== canvas) nodes.push(n);
    }
  ...
  function boot() {
    absorbWords();
    if (!solid.querySelector(".ai-word")) {
      setTimeout(boot, 80);
      return;
    }
